...
export default function HighSchoolSuggestionBoxWebsite() {
  ...
  const loginUsers = selectedLoginRole ? MOCK_USERS.filter((user) => user.role === selectedLoginRole) : [];
        ...
        {page === "home" && (
          <div className="space-y-8">
            ...
            <section className="grid gap-4 md:grid-cols-4">
              {[
                [GraduationCap, "School Login", `Students and STUCO admins use ${SCHOOL_DOMAIN} accounts.`],
                [ShieldCheck, "Role Access", "Students and STUCO admins see different tools."],
                [Users, "One Signature", "Each student account can sign a petition only once."],
                [CheckCircle2, "Escalation", "Petitions reaching 50 signatures go back to STUCO."],
              ].map(([Icon, title, text]) => (
                <FeatureCard key={String(title)} icon={Icon as LucideIcon} title={String(title)} text={String(text)} />
              ))}
            </section>
          </div>
        )}
          ...
          <section className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            {isSupabaseConfigured ? (
              <div className="mt-6 grid gap-3">
                <button onClick={signInWithGoogle} className="flex items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white px-6 py-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  <div>
                    <div className="text-base font-black leading-tight">Continue with Google</div>
                    <div className="mt-2 text-sm font-semibold leading-tight text-slate-500">Only {SCHOOL_DOMAIN} accounts are accepted.</div>
                  </div>
                  <ArrowRight className="shrink-0 text-slate-500" />
                </button>
                <div className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                  STUCO admin access is assigned by the configured admin email list, not by selecting a role on this screen.
                </div>
              </div>
            ) : (
              <>
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  <button
                    ...
                    className={classNames(
                      ...
                      selectedLoginRole === "student" ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white"
                    )}
                  >
                    <div className="font-black">Student</div>
                    <div className="mt-1 text-sm font-semibold text-slate-500">Submit, sign, comment, and track ideas.</div>
                  </button>
                  <button
                    onClick={() => setSelectedLoginRole("stuco_admin")}
                    className={classNames(
                      "rounded-2xl border p-4 text-left transition hover:bg-slate-50",
                      selectedLoginRole === "stuco_admin" ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white"
                    )}
                  >
                    <div className="font-black">Student Council Admin</div>
                    <div className="mt-1 text-sm font-semibold text-slate-500">Review submissions, petitions, and comments.</div>
                  </button>
                </div>

                <div className="mt-6 grid gap-3">
                  {selectedLoginRole ? loginUsers.map((user) => (
                    <button
                      key={user.id}
                      onClick={() => {
                    ...
                    <div className="rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-600">Choose a role above to continue.</div>
                  )}
                </div>

                <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                  Prototype fallback: mock users are shown because Supabase environment variables are not configured.
                </div>
              </>
            )}
          </section>
